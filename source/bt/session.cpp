...
public:
    ...
    void apply_announce(const AnnounceResult& result, uint64_t now_ms) {
        ...
        // plus. La planification provisoire posée à l'émission (court, pour
        // pouvoir réessayer vite en cas d'échec) est ici remplacée par la
        // vraie.
    ...
    TorrentStatus status() const {
        ...
        s.pieces_total = meta_complete_ ? meta_.piece_count() : 0;
        s.pieces_done = meta_complete_ ? picker_.have().count() : 0;
        s.corrupt_pieces = corrupt_pieces_;
        s.files_count = static_cast<uint32_t>(meta_.files.size());
...
bool Session::add_torrent_file(const std::string& path, std::string* err) {
    ...
    return add_torrent_data(blob, err);
}

bool Session::add_torrent_data(const std::string& blob, std::string* err, std::string* name_out) {
    MetaInfo meta;
    if (!parse_torrent(blob, meta, err)) return false;

    const std::string hash_hex = util::to_hex(meta.info_hash);
    if (name_out) *name_out = meta.name;
    if (has_torrent(hash_hex)) {
        if (err) *err = "torrent déjà présent";
        return false;
    }
    ...
    remember_magnet(hash_hex, magnet_for(meta));
    cmd.meta = std::move(meta);
